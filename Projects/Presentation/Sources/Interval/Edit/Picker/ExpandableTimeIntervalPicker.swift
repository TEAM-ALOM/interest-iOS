    ...
    func pickIntervalView (selection: Binding<Interval>, isExpanded: Binding<Bool>) -> some View {
        VStack {
            ...
            Picker("Interval", selection: selection) {
                Text("1구간 (123~134BPM)").tag(Interval.section1)
                Text("2구간 (135~148BPM)").tag(Interval.section2)
                Text("3구간 (149~162BPM)").tag(Interval.section3)
                Text("4구간 (163~175BPM)").tag(Interval.section4)
                Text("5구간 (176BPM~)").tag(Interval.section5)
            }
            .pickerStyle(.wheel)
